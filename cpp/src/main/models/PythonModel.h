...
public:
  PythonModelRankingScoreIterator(PythonRankingIteratorModel* model, int user){
    model_ = model;
    user_ = user;
  }
...
class PythonRankingIteratorModel : public PythonModel, public RankingScoreIteratorProvider
 {
public:
  PythonRankingIteratorModel() : iterator_(this, -1) {}
  virtual RankingScoreIterator* get_ranking_score_iterator(int user){
    iterator_ = PythonModelRankingScoreIterator(this, user);
    return &iterator_;
  }
  virtual bool iterator_has_next_(int user, double upper_bound) = 0;
  virtual pair<int, double> iterator_get_next_(int user) = 0;
protected:
