...
vector<RecDat>* PredictionCreatorGlobal::run(RecDat* rec_dat){
  ...
  for(int ii=(int)min_heap_->size()-1; ii>=0; ii--){
    top_predictions_[ii]=min_heap_->getMin();
    min_heap_->deleteMin();
  }
...
void PredictionCreatorGlobal::process_rectangle(vector<pair<int,double> >* sorted_users, vector<pair<int,double> >* sorted_items, uint begin_user_index, uint begin_item_index, uint end_user_index, uint end_item_index, RecDat* fake_rec_dat){
  ...
  while(user_index<sorted_users->size() and item_index<sorted_items->size() and user_index<end_user_index and item_index<end_item_index){
    ...
    if(min_heap_->size()==top_k_ and (user_bound<min_heap_->getMin().score or item_bound<min_heap_->getMin().score)){break;}
...
void PredictionCreatorGlobal::process_line(vector<pair<int,double> >* sortedAs,uint beginAIndex, uint endAIndex, int* fake_rec_dat_a, RecDat* fake_rec_dat){
  for(uint aIndex=beginAIndex;aIndex<min(endAIndex,(uint)sortedAs->size());aIndex++){
    double aBound = (*sortedAs)[aIndex].second;
    if(min_heap_->size()==top_k_ and aBound<=min_heap_->getMin().score){break;}
    (*fake_rec_dat_a)=(*sortedAs)[aIndex].first;
...
vector<RecDat>* PredictionCreatorPersonalized::run(RecDat* rec_dat){ //TODO test
  ...
  for(auto item_bound: *sorted_items){
    if(min_heap_->size()==top_k_ and item_bound.second!=-1 and item_bound.second<min_heap_->getMin().score){ break; } //all consecutive items have lower prediction
