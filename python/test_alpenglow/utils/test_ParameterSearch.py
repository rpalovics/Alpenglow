...
import math
import unittest
from alpenglow.utils import ParameterSearch


class TestParameterSearch(unittest.TestCase):
    def test__get_configurations(self):
        c = ParameterSearch(None, None)
        c.set_parameter_values('negative_rate', [1, 2, 3])
        c.set_parameter_values('learning_rate', [0.05, 0.1])

        params = c._get_configurations()
    ...
    def test_runOneConfig(self):
        ...
        assert r['DcgScore'][0] - sum(dcgs) / len(dcgs) < 0.000000001
    ...
    def test_runMultiple(self):
        ...
        for (k, s, v) in r.values:
            if k == 100 and s == 254938879:
                self.assertAlmostEqual(v, sum(dcgs) / len(dcgs))
                hit += 1
        assert hit == 1
